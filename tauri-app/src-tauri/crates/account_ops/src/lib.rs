...
/// 软件内「添加账号」：把账号登记写入本机 WorkBuddy 登录态文件的 `allAccounts`，
/// 使其立刻出现在账户中枢侧边栏（即使尚未真正登录也能被管理/备注）。
///
/// - `uid`：必填，账号唯一标识
/// - `nickname`：可选昵称（不填则留空，后续可被 WorkBuddy 自动补全）
/// - `token`：可选 accessToken；若提供，会一并写入该账号条目，并可生成 vault 快照，
///   使该账号「立即可切换」（无需先在 WorkBuddy 里登录）
/// - `make_snapshot`：是否在提供了 token 时生成可切换的 vault 快照（默认 true）
///
/// 实现：合并式写入——只改 `allAccounts` 中该 uid 的条目（去重），不动其它已登记账号；
/// 写回后再保证 vault/<uid>/snapshot/ 存在（有 token 时生成完整快照，无 token 时仅登记）。
#[derive(Serialize)]
...
pub fn add_account(
    ...
    // 1) 读取当前登录态文件（workbuddy-desktop.info）
    let af = auth_file().ok_or_else(|| {
        "未找到本机 WorkBuddy 登录态文件（workbuddy-desktop.info），请先在本机登录过 WorkBuddy".to_string()
    })?;
    let s = std::fs::read_to_string(&af).map_err(|e| format!("读取登录态失败: {e}"))?;
    let mut v: Value = serde_json::from_str(&s).map_err(|e| format!("登录态 JSON 解析失败: {e}"))?;

    // 2) 构造要写入的账号条目
    let mut entry = json!({ "uid": uid });
    if let Some(n) = nickname {
        if !n.trim().is_empty() {
            entry["nickname"] = json!(n.trim());
        }
    }
    if let Some(t) = token {
        if !t.trim().is_empty() {
            entry["accessToken"] = json!(t.trim());
            entry["token"] = json!(t.trim());
        }
    }

    // 3) 合并进 allAccounts（去重：相同 uid 的旧条目整体替换）
    let arr = v
        .get_mut("allAccounts")
        .and_then(|x| x.as_array_mut())
        .cloned()
        .unwrap_or_default();
    let existed = arr.iter().any(|x| x.get("uid").and_then(|u| u.as_str()) == Some(uid));
    let mut merged: Vec<Value> = arr
        .into_iter()
        .filter(|x| x.get("uid").and_then(|u| u.as_str()) != Some(uid))
        .collect();
    merged.push(entry.clone());
    v["allAccounts"] = Value::Array(merged);

    // 4) 写回登录态文件（原子写：临时文件 + rename）
    let tmp = af.with_extension("info.tmp");
    let out = serde_json::to_string_pretty(&v).map_err(|e| format!("序列化失败: {e}"))?;
    std::fs::write(&tmp, &out).map_err(|e| format!("写入失败: {e}"))?;
    std::fs::rename(&tmp, &af).map_err(|e| format!("替换失败: {e}"))?;

    // 5) 生成/登记 vault 快照（有 token 时生成完整可切换快照；否则仅保证目录存在以便管理）
    let mut added_snapshot = false;
    if token.as_deref().map(|t| !t.trim().is_empty()).unwrap_or(false) && make_snapshot {
        if let Err(e) = materialize_snapshot_for(vault, uid) {
            // 快照失败不致命：账号已登记成功，只是暂时不可切换
            return Ok(AddAccountResult {
                success: true,
                message: format!("账号已登记，但生成可切换快照失败（仍可管理，切换需先在 WorkBuddy 登录）: {e}"),
                uid: uid.to_string(),
    ...
    let msg = if existed {
        "已更新该账号登记信息".to_string()
    } else {
        if added_snapshot {
            "已添加账号并生成可切换快照，可立即切换".to_string()
        } else {
            "已添加账号到列表（尚未登录，切换前请先在 WorkBuddy 登录该账号）".to_string()
        }
    };
